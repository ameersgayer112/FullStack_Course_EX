...
var b1 = Book.find({ pages: { "$lt": 500, "$gt": 200 } })

b1.exec(function (err, books) {
...
var b2 = Book.find({ rating: { "$lt": 5 } }).sort({ author: -1 })
b2.exec(function (err, books) {
...
var b3 = Book.find({ genres: "Fiction" }).skip(2).limit(3);
b3.exec(function (err, books) {
...
var p1 = Person.find({ height: { "$gt": 180 }, salary: { "$gt": 30000 } })
p1.exec(function (err, persons) {
...
var p2 = Person.find({ $or: [{ height: { "$gt": 180 } }, { salary: { "$gt": 30000 } }] })
p2.exec(function (err, persons) {
...
var p3 = Person.find({$and:[
  { $or: [{ hair: "grey" }, { eyes: "grey" }] },
  { weight: { "$lt": 70 } }
]})
p3.exec(function (err, persons) {
...
var p4 = Person.find({ kids: { $eachelement: { hair: "grey" } } })
p4.exec(function (err, persons) {

  for (p in persons) {
    let kidPerson = persons[p]
    console.log("Person", p, "Have at least 1 kid with grey hair and the kid name is", kidPerson.kids)
  }

})

//5. Find all the people who have at least one overweight kid, and are overweight themselves (>100)
var p5 = Person.find({ $and:[
  { weight: { "$gt": 100 } },
  { kids: { $elemMatch: { weight: { "$gt": 100 } } } }
]})
p5.exec(function (err, persons) {

for(p in persons){
  let person = persons[p]
  console.log("\nPerson", p, "Has Over Weight" , person.weight , "and kids" , person.kids)
